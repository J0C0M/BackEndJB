...
@section('content')
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Nieuwe categorie toevoegen</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.categories.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="shadow-sm focus:ring-purple-500 focus:border-purple-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('description') }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
